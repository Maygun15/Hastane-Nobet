// services/llmService.js — Anthropic/OpenAI adaptörü + AILog kaydı
...
async function llmChat({ systemPrompt, messages, maxTokens = 512, action = 'chat', hospitalId, userId }) {
  ...
  try {
    result = AI_PROVIDER === 'openai'
      ? await callOpenAI({ systemPrompt, messages, maxTokens })
      : await callAnthropic({ systemPrompt, messages, maxTokens });
    return result;
  } catch (err) {
    success = false;
